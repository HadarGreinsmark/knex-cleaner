...
function isClean (knex, passedInOptions) {
  var options = _.defaults({}, passedInOptions, DefaultOptions);
  ...
  .then(function(tables) {
    ...
    return Promise.map(tables, function(tableName) {
      var tableCounts = [];
      tableCounts.push( knexTables.getTableRowCount(knex, tableName) );

      return tableCounts.map(tableCounts, function (count) {
        if (count.length != 0)
          throw new Error() ;
      }).then(function () {
        return true;
